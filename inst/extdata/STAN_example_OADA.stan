//stan
 data {
    ...
    array[K, T_max] matrix[P, P] A_default; // Network matrices
...
model {
    ...
    for (trial in 1:K) {
        for (n in 1:N[trial]) {
            ...
            if (learn_time > 0) {
                real i_ind = 1.0;
                real i_soc = s * (sum(A_default[trial, time_step][id, ] .* Z[trial][time_step, ])) ;
                real i_lambda = 1.0 * (i_ind + i_soc);
                vector[Q] j_rates = rep_vector(0.0, Q);
                for (j in 1:Q) {
                    real j_ind = 1.0;
                    real j_soc = s * (sum(A_default[trial, time_step][j, ] .* Z[trial][time_step, ])) ;
