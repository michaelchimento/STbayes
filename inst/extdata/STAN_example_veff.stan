...
real lambda_0_mean = 1 / exp(log_lambda_0_mean);
vector<lower=0>[Z] s_prime = 1 / exp(log_s_mean + v_ID[,2]);
vector<lower=0>[Z] s = s_prime ./ lambda_0;
real<lower=0> s_mean = (1 / exp(log_s_mean)) / (1 / exp(log_lambda_0_mean));
}
model {
    log_lambda_0_mean ~ uniform(-10, 10);
    log_s_mean ~ normal(2,3);
        ...
        for (n in 1:N[trial]) {
            ...
            if (learn_time > 0) {
                for (time_step in 1:learn_time) {
                    real ind_term = 1.0;
                    real soc_term = s_prime[id] * (sum(A_assoc[trial, time_step][id, ] .* C[trial][time_step, ])) ;
