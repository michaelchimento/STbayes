test_that("Network structure is consistent using distribution edgeweights.", {
    ...
    event_data$id <- as.character(event_data$id)

    # Import data
    data_imported <- import_user_STb(event_data, networks, network_type = "directed")
    ...
    # Import data
    data_imported <- import_user_STb(event_data, list(networks, networks))
    ...
    # Import data
    # data_imported <- import_user_STb(event_data, STbayes::strand_results_obj)
    # testthat::expect_equal(length(data_imported$logit_edge_mu), 90)
    # testthat::expect_equal(dim(data_imported$logit_edge_cov), c(1, 90, 90))
})
