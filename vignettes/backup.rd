...
- ```time```: Integer or float values indicating the time (TADA) or order (OADA) in which the individual was recorded as first informed/knowledgable. If an individual had the event occur prior to the start of the observation period (e.g. pre-trained demonstrator), set as 0. These left censored individuals will not contribute to the likelihood calculation. If an individual never learned during the observation period, set its value $t_{\text{end}}+1$. These will be treated as right-censored individuals in the likelihood calculation.
- ```t_end```: Integer or float value representing the duration of the observation period for each trial. If the user observed a population for 30 days, \verb|t_end=30|. If there are no censored individuals set \verb|t_end=max(time)|.
...
Any remaining columns are descriptively named integer or float edge weights. If you were making a multi-network model, you could add as many columns as you want. Here, there is only one network named ```assoc```.  Naming conventions for identities and trials must be consistent between these two dataframes (i.e. integers, or matching character strings). The networks dataframe is used as the reference for all unique IDs, thus each ID must be included at least once in either the focal or other column. If a dyad is absent, their connection is assumed to be zero. 
...
Here is a summary of each argument to ```generate_STb_model``` for easy reference:
...
- \verb|veff_ID=c("lambda_0", "s", ...),| is used to specify which variables should receive varying effects by individual. For example, \verb|veff_ID=c("lambda_0", "s")| will create a model that will attempt to fit varying effects for both intrinsic rate and strength of social transmission. It is possible to include other ILVs, if desired. 
- \verb|model_type=c("full","asocial")| specifies whether to create a full model or an asocial model constrained to the intrinsic rate only
- \verb|intrinsic_rate=c("constant", "weibull")| specifies whether or not the user wishes to use a constant intrinsic rate, or a Weibull shaped rate that can increase or decrease over time. This fits an additional parameter $\gamma$, where $\gamma<1$ indicates a decreasing hazard over time, and $\gamma>1$ indicates an increasing hazard over time. $\gamma=1$ indicates a constant hazard.
- \verb|transmission_func=c("standard","freqdep_f", "freqdep_k")| specifies whether the model uses simple or complex transmission.
- \verb|gq| specifies whether or not a generated quantities block is created in the Stan model. This defaults to true, as the log-likelihood of observations is output using generated quantities and is needed for model comparison.
- \verb|est_acqTime| specifies whether estimated acquisition times are also output by the generated quantities block. These are useful for posterior predictive checks, but this argument defaults to false for efficiency in fitting.
- \verb|priors| allows users to specify priors for parameters in the format of a named list. Please see the advanced recipes vignette for more details
...
Dynamic networks that change over the observation period may also be input by adding an optional column \enquote{time} containing integers indicating which timestep the edges belong to. The network should be summarized for each inter-event period. For example, if the user's observation period included 10 events, they should supply edge weights from 11 networks in total, where \verb|time=1| should contain the network representing the period from $[t_0, t_{e1})$, \verb|time=2| represents $[t_{e1}, t_{e2})$, and \verb|time=11| represents from $[t_{e10}, t_{\text{end}}]$. 
...
For $lambda_0$ and $s$, varying effects are added onto the main effect prior to transformation from log-scale back to linear. For example, if we apply a varying effect for $\lambda_0$, the model will calculate a value for each individual in the \verb|transformed parameters| block of the Stan model:
