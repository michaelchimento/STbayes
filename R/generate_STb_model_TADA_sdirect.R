...
generate_STb_model_TADA_sdirect <- function(STb_data,
    ...
    if (model_type == "full") {
        if (separate_s) {
            s_param <- "vector[N_networks] log_s_direct_mean;"
        } else {
            s_param <- "real log_s_direct_mean;"
        }
    } else {
        s_param <- ""
    }
    ...
    if (!is.element("s", veff_ID) & model_type == "full") {
        if (separate_s) {
            # static s[n]
            transformed_params <- append(transformed_params, "vector<lower=0>[N_networks] s_direct = exp(log_s_direct_mean);")
            gq_transformed_params <- append(gq_transformed_params, "vector<lower=0> s_prime = s_direct .* lambda_0;")
        } else {
            # static scalar s
            transformed_params <- append(transformed_params, "real<lower=0> s_direct = exp(log_s_direct_mean);")
            if (!is.element("lambda_0", veff_ID)) {
                gq_transformed_params <- append(gq_transformed_params, "real<lower=0> s_prime = s_direct*lambda_0;")
            } else {
                gq_transformed_params <- append(gq_transformed_params, "vector<lower=0>[P] s_prime = s_direct*lambda_0;")
            }
        }
    }
...
model {{
    ...
    {if (model_type=='full') paste0('log_s_direct_mean ~ ',prior_s,';') else ''}
