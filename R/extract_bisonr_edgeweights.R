...
#'
#' @param bison_fit object of class "bison_model"
...
#'
#' @examples
extract_bisonr_edgeweights <- function(bison_fit, draws=100){

    if (!inherits(bison_fit, "bison_model")) stop("Please supply a bisonr fit object.")

    #yoinked code from bisonr so as not to depend
    samples <- bison_fit$edge_samples
    draw_ids <- sample(1:nrow(samples), draws, replace = FALSE)
    edge_samples <- samples[draw_ids, ]

    if (bison_fit$directed){
        dyads <- strsplit(bison_fit$dyad_names, " -> ") # split "from-to"
    } else {
        dyads <- strsplit(bison_fit$dyad_names, " <-> ") # split "from-to"
    }
    ...
    draw_index <- rep(1:draws, each=bison_fit$num_dyads)
