...
#' @param priors named list with strings containing the prior for log baserate, s or f. defaults to list(log_lambda_0 = "uniform(-10, 10)", log_s = "uniform(-10, 10)", log_f = "normal(0,1)")
...
generate_STb_model_cTADA <- function(STb_data,
                               ...
                               gq = TRUE,
                               est_acqTime = FALSE,
                               priors = list(log_lambda_0 = "uniform(-10, 10)",
                                             log_s = "uniform(-10, 10)",
                                             log_f = "normal(0,1)")) {
    ...
    if (!is.element('s', veff_ID) & model_type=="full"){
        transformed_params = append(transformed_params,"real<lower=0> s_prime = 1 / exp(log_s_mean);")
        transformed_params = append(transformed_params,"real<lower=0> s = s_prime/lambda_0;")
    }
...
")
    ...
    if (model_type=='full'){
        social_info_statement = glue::glue("real soc_term = {if (is.element('s', veff_ID)) 's_prime[id]' else 's_prime'} * ({network_term}) {ILVs_variable_effects};")
